			...
			if (mapinfo||zmapinfo) {
				...
				if ((buf=mz_zip_reader_extract_to_heap(&zip_archive, mapinfo_idx, &buf_len, 0))) {
					...
					foreach (const QString &str, QString(char_buf).split(QRegularExpression("[\r\n]"), Qt::SkipEmptyParts)) {
						QRegularExpression name_re("^\\s*map\\s+([^\\s]+)(\\s+.*)?$", QRegularExpression::CaseInsensitiveOption);
						QRegularExpressionMatch name_m=name_re.match(str);
						if (name_m.hasMatch())
							map_names<<name_m.captured(1).left(8).toUpper();
					}
				...
				if (!mz_zip_reader_is_file_a_directory(&zip_archive, i)&&mz_zip_reader_file_stat(&zip_archive, i, &file_stat)) {
					...
					if (!zname.baseName().compare("iwadinfo", Qt::CaseInsensitive)&&!zname.path().compare(".")) {
						...
						if ((buf=mz_zip_reader_extract_to_heap(&zip_archive, i, &buf_len, 0))) {
							...
							QRegularExpression name_re("\\s+Name\\s*=\\s*\"(.+?)\"\\s+", QRegularExpression::CaseInsensitiveOption);
							QRegularExpressionMatch name_m=name_re.match(QString::fromUtf8(char_buf));
							if (name_m.hasMatch())
								iwad_name=name_m.captured(1);
