...
ZDLConf::ZDLConf(int mode)
{
	...
	this->mode = mode;
	//cout << "New configuration" << Qt::endl;
	reads = 0;
	writes = 0;
	mutex = LOCK_BUILDER();
...
ZDLConf::~ZDLConf()
{
	...
	//cout << "Configuration deleted." << Qt::endl;
	...
	//if (vars)
	//	delete vars;
	//cout << "Deleting configuration children." << Qt::endl;
...
int ZDLConf::setValue(QString lsection, QString variable, QString szBuffer)
{
	...
	if((mode & WriteOnly) != 0){
		...
		//cout << "int ZDLConf::setValue("<<lsection<<","<<variable<<","<<szBuffer<<")"<<Qt::endl;
		section->setValue(variable, value);
		releaseWriteLock();
	}
	return 0;
