...
ZDLInterface::ZDLInterface(QWidget *parent):ZDLWidget(parent){
	...
	//A QSplitter distributes by sizeHint, and a file list's hint grows with
	//the longest path it holds, which hands nearly everything to the external
	//file list and squeezes the settings pane down to nothing. Its own width
	//is meaningless until the first layout pass, so the split is applied from
	//a zero timer once the widget has been laid out.
	QTimer::singleShot(0, this, SLOT(balanceTopPane()));
...
QLayout *ZDLInterface::getTopPane(){
	...
	//The library and the presets are both pickers and neither needs much
	//height, so they share one column rather than taking two. That leaves
	//three columns across instead of four.
	leftStack = new QSplitter(Qt::Vertical, rsplit);

	ZDLPwadList *ppane = new ZDLPwadList(leftStack);
	ZDLPresetList *xpane = new ZDLPresetList(leftStack);
	ZDLFilePane *fpane = new ZDLFilePane(rsplit);
	ZDLSettingsPane *spane = new ZDLSettingsPane(rsplit);

	leftStack->addWidget(ppane);
	leftStack->addWidget(xpane);
	leftStack->setStretchFactor(0, 3);
	leftStack->setStretchFactor(1, 2);

	//addChild does the layout and the configuration wiring together, and
	//these two are laid out by leftStack now. They still have to sit in the
	//widget tree that carries newConfig and rebuild around, so say so.
	ppane->setZParent(split);
	xpane->setZParent(split);

	split->addChild(fpane);
	split->addChild(spane);

	//A QSplitter lays its children out by sizeHint, and a file list's hint
	//grows with the longest path in it, which would hand almost everything to
	//the external file list and push the settings pane off screen. Give the
	//panes a small floor and state the split explicitly instead.
	ppane->setMinimumWidth(90);
	xpane->setMinimumWidth(80);
	leftStack->setMinimumWidth(90);
	fpane->setMinimumWidth(120);
	spane->setMinimumWidth(180);

	//The stacked pickers are for choosing; the external file list and the
	//settings pane are where the work happens, so they take the extra width.
	rsplit->setStretchFactor(0, 2);
	rsplit->setStretchFactor(1, 3);
	rsplit->setStretchFactor(2, 3);
...
void ZDLInterface::balanceTopPane(){
	if (!topSplit)
		return;

	//Same problem vertically: the tree's height hint swallows the column and
	//leaves the presets a sliver, so state the split unless one was stored.
	if (leftStack&&!restoreSplitterSizes("splitleft", leftStack)) {
		int height=leftStack->height();
		if (height>200) {
			QList<int> stack;
			stack<<height*60/100<<height*40/100;
			leftStack->setSizes(stack);
		}
	}

	//Whatever the panes were dragged to last time wins over the default. A
	//layout stored by the four column build has the wrong number of entries
	//and is ignored, so this falls through to the new default once.
	if (restoreSplitterSizes("splitlaunch", topSplit))
		return;

	int width=topSplit->width();
	if (width<200)
		return;

	//Pickers, external files, settings.
	QList<int> sizes;
	sizes<<width*26/100<<width*36/100<<width*38/100;
	topSplit->setSizes(sizes);
}
...
void ZDLInterface::importCurrentConfig(){
	...
	QMessageBox::StandardButton btnrc = QMessageBox::warning(this, ZDL_APP_NAME, text, QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
	...
	if(!userConf.exists()){
		...
		if(!dir.exists()){
			...
			if(!dir.mkpath(dir.absolutePath())){
				...
				QMessageBox::critical(this, ZDL_APP_NAME, QString("Unable to create the directories for a global user configuration at ")+dir.absolutePath());
				return;
			}
		}
	}
	...
	if(zconf->writeINI(userConfPath) != 0){
		...
		QMessageBox::critical(this, ZDL_APP_NAME, QString("Unable to write the configuration file at ")+userConfPath);
		return;
	}
...
QLayout *ZDLInterface::getButtonPane(){
	...
	btnZDL = new QPushButton(ZDL_APP_NAME, this);
...
void ZDLInterface::showCommandline(){
	...
	if (exec.isEmpty()) {
		QMessageBox::critical(this, ZDL_APP_NAME, "Please select a source port");
		return;
	}
...
void ZDLInterface::rebuild(){
	...
	saveSplitterSizes("splitlaunch", topSplit);
	saveSplitterSizes("splitleft", leftStack);
	ZDLConf *zconf = ZDLConfigurationManager::getActiveConfiguration();
