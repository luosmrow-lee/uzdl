...
ZDLMapFile *ZDLMapFile::getMapFile(QString file)
{
	...
	QRegularExpression ban_exts(QRegularExpression::anchoredPattern("lmp|txt|cfg|ini|deh|bex|zdl|zds|dsg|esg"), QRegularExpression::CaseInsensitiveOption);	//Blacklist obvious non-map files

	if (file_info.isDir()) {
		mapfile=new ZLibDir(file);
	} else if (ext.length()&&!ban_exts.match(ext).hasMatch()&&file_info.exists()) {	//Only process files with present non-blacklisted extension
		QFile fileio(file);
