...
//Where the file dialogs start: the usual IWAD folder when one is known,
//or the iwads folder beside uZDL while there is none yet.
static QString IwadStartDir(bool dwd_first)
{
	QString dir=getWadLastDir(NULL, dwd_first);
	if (dir.isEmpty()&&QDir(ZDLPaths::iwadsDir()).exists())
		dir=ZDLPaths::iwadsDir();
	return dir;
}
...
void ZDLIWadList::wizardAddButton(){
	ZDLIwadInfo zdl_fi;
	ZDLNameInput diag(this, IwadStartDir(true), &zdl_fi, true, false);
	diag.setWindowTitle("Add IWAD");
	diag.setFilter(iwad_filters);
	if (diag.exec()){
		saveWadLastDir(ZDLPaths::resolve(diag.getFile()));
...
void ZDLIWadList::newDrop(QStringList fileList){
	...
	for (int i=0; i<fileList.size(); i++)
		insert(new ZDLNameListable(pList, 1001, ZDLPaths::preferRelative(fileList[i]), ZDLIwadInfo(fileList[i]).GetFileDescription()), -1);
}

void ZDLIWadList::addButton(){
	LOGDATAO() << "Adding new IWADs" << Qt::endl;

	QStringList fileNames = QFileDialog::getOpenFileNames(this, "Add IWADs", IwadStartDir(false), iwad_filters);
	for(int i = 0; i < fileNames.size(); i++){
		LOGDATAO() << "Adding file " << fileNames[i] << Qt::endl;
		saveWadLastDir(fileNames[i]);
		insert(new ZDLNameListable(pList, 1001, ZDLPaths::preferRelative(QFD_QT_SEP(fileNames[i])), ZDLIwadInfo(fileNames[i]).GetFileDescription()), -1);
	}
}
