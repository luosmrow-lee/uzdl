	...
	QListWidgetItem(text, parent), prog_id(prog_id), desc(desc), extensions(exts.split(" ")), icon(icon), hklm(hklm), remove(false)
{
	...
	foreach (const QString &ext, extensions) {
		SimpleWFA::AssocStatus cur_stat=SimpleWFA::CheckAssociationStatus(hklm, true, WStr(ext), WStr(prog_id), NULL);
		if (initial) {
			res_stat=cur_stat;
			initial=false;
...
void AssocListWidget::Process(const QString &file_path)
{
	...
	if (!remove&&changed&&checkState()==Qt::Checked) {
		foreach (const QString &ext, extensions) {
			SimpleWFA::Associate(hklm, WStr(ext), WStr(prog_id));
		}
		SimpleWFA::CreateSimpleProgID(hklm, SimpleWFA::CD_UPDATE_ALWAYS, WStr(prog_id), WStr(file_path), WStr(desc), icon);
	}
	
	if (remove||(changed&&checkState()==Qt::Unchecked)) {
		foreach (const QString &ext, extensions) {
			SimpleWFA::Deassociate(hklm, WStr(ext), WStr(prog_id));
		}
	}

	if (remove) {
		SimpleWFA::RemoveProgID(hklm, WStr(prog_id));
	}
}
	...
	QDialog(parent)
{
	...
	assoc_list->addItem(new AssocListWidget("Demo files (*.lmp)", assoc_list, "ZDL.Demo.1", "Doom engine demo file", ".lmp", hklm, IDI_DEMO));
