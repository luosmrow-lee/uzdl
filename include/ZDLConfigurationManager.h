...
//Pane widths, stored as comma separated pixel sizes under zdl.general.
//restoreSplitterSizes returns false when there is nothing usable stored, so
//a caller can fall back on whatever default split it would otherwise apply.
class QSplitter;
void saveSplitterSizes(const QString &key, QSplitter *split, ZDLConf *zconf=NULL);
bool restoreSplitterSizes(const QString &key, QSplitter *split, ZDLConf *zconf=NULL);
